...
from shop.api.schema import CitizenPresentsResponseSchema
from shop.db.schema import (
    units_table as citizens_t, relations_table as relations_t,
)
...
class CitizenBirthdaysView(BaseImportView):
    ...
    async def get(self):
        ...
        for month, rows in groupby(rows, key=lambda row: row['month']):
            for row in rows:
                result[month].append({'unit_id': row['unit_id'],
