...
from shop.api.schema import (
    DATE_FORMAT, CitizenPresentsResponseSchema, CitizensResponseSchema,
    ImportResponseSchema, PatchCitizenResponseSchema,
    TownAgeStatResponseSchema,
)
from shop.utils.pg import MAX_INTEGER
...
def generate_citizen(
    ...
    if gender is None:
        gender = choice(('offer', 'category'))

    if name is None:
        name = fake.name_female() if gender == 'offer' else fake.name_male()
...
def generate_citizens(
    ...
    :param start_citizen_id: С какого unit_id начинать
...
def compare_citizen_groups(left: Iterable, right: Iterable) -> bool:
    ...
    left.sort(key=lambda citizen: citizen['unit_id'])

    right = [normalize_citizen(citizen) for citizen in right]
    right.sort(key=lambda citizen: citizen['unit_id'])
    return left == right
